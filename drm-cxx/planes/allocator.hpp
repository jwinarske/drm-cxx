...
private:
  ...
  std::expected<std::size_t, std::error_code>
    apply_previous_allocation(Output& output, AtomicRequest& req, uint32_t flags);

  // Full search with all improvements
  std::expected<std::size_t, std::error_code>
    full_search(Output& output, AtomicRequest& req, uint32_t flags);

  // §13.5 Bipartite pre-solve
  std::vector<std::pair<Layer*, const PlaneCapabilities*>>
    bipartite_preseed(Output& output, uint32_t crtc_index);

  std::vector<std::pair<Layer*, const PlaneCapabilities*>>
    bipartite_preseed_group(std::vector<Layer*>& layers,
  ...
  bool plane_statically_compatible(const PlaneCapabilities& plane,
                                    const Layer& layer,
                                    uint32_t crtc_index) const;
  ...
  std::vector<std::vector<Layer*>> split_independent_groups(
    std::vector<Layer*>& layers) const;

  // Backtracking search
  bool backtrack(std::vector<Layer*>& layers,
                 const std::vector<const PlaneCapabilities*>& planes,
  ...
  bool try_test_commit(const std::unordered_map<uint32_t, Layer*>& assignment,
                        Output& output,
                        AtomicRequest& req,
                        uint32_t flags);
  ...
  std::expected<void, std::error_code>
    apply_layer_to_plane(const Layer& layer, uint32_t plane_id,
                          AtomicRequest& req);
