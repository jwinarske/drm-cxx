...
auto* li(void* p) { return static_cast<struct libinput*>(p); }
auto* ud(void* p) { return static_cast<struct udev*>(p); }
...
Seat::Seat(Seat&& other) noexcept
  : li_(other.li_), udev_(other.udev_),
    handler_(std::move(other.handler_)), fd_(other.fd_) {
  other.li_ = nullptr;
  other.udev_ = nullptr;
  other.fd_ = -1;
}
...
Seat::open(SeatOptions opts) {
  ...
  seat.li_ = libinput_udev_create_context(
    &li_interface, nullptr, ud(seat.udev_));
  if (!seat.li_) {
    return std::unexpected(std::make_error_code(std::errc::no_such_device));
  }
...
void Seat::process_events() {
  ...
  while ((ev = libinput_get_event(li(li_))) != nullptr) {
    ...
    case LIBINPUT_EVENT_SWITCH_TOGGLE: {
      ...
      se.active = libinput_event_switch_get_switch_state(sev) ==
