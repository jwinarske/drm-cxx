...
class Layer {
 public:
  ...
  Layer(LayerHandle handle, std::unique_ptr<LayerBufferSource> source,
        const DisplayParams& display, drm::planes::ContentType content_type,
        std::uint32_t update_hint_hz) noexcept
      : handle_(handle),
