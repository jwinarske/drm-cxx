...
int main() {
  ...
  for (const auto& d : display.displays()) {
    std::println("  Display '{}': {}x{} (handle=0x{:x})",
      d.name, d.width, d.height, d.display_handle);

    auto planes = display.planes_for_display(d.display_handle);
    for (const auto* p : planes) {
      std::println("    Plane {}: stack_index={}",
        p->plane_index, p->current_stack_index);
    }
  }

  std::println("\nVulkan display planes: {}", display.planes().size());
  for (const auto& p : display.planes()) {
    std::println("  Plane {}: stack_index={}, supported_displays={}",
      p.plane_index, p.current_stack_index, p.supported_displays.size());
  }
