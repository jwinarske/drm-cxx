...
using drm::csd::compute_damage;
using drm::csd::DamageRect;
using drm::csd::DamageSlot;
using drm::csd::PlaneSlot;
using drm::csd::PropertyWrite;

DamageRect damage(const std::vector<DamageSlot>& prev, const std::vector<DamageSlot>& cur,
                  std::uint32_t cw, std::uint32_t ch) {
  return compute_damage(drm::span<const DamageSlot>(prev.data(), prev.size()),
                        drm::span<const DamageSlot>(cur.data(), cur.size()), cw, ch);
}

DamageSlot armed(std::int32_t x, std::int32_t y, std::uint32_t w, std::uint32_t h,
                 std::uint64_t gen = 0) {
  return DamageSlot{true, x, y, w, h, gen};
}
...
TEST(CsdCompositeDamage, FirstFrameDamagesWholeCanvas) {
  const std::vector<DamageSlot> prev;  // empty = first frame
  const std::vector<DamageSlot> cur{armed(0, 0, 100, 100)};
...
TEST(CsdCompositeDamage, SlotCountChangeDamagesWholeCanvas) {
  const std::vector<DamageSlot> prev{armed(0, 0, 100, 100)};
  const std::vector<DamageSlot> cur{armed(0, 0, 100, 100), armed(200, 0, 100, 100)};
  const auto d = damage(prev, cur, 1920, 1080);
  EXPECT_EQ(d.w, 1920U);
  EXPECT_EQ(d.h, 1080U);
}
...
TEST(CsdCompositeDamage, NoChangeIsEmpty) {
  const std::vector<DamageSlot> slots{armed(10, 10, 100, 100, 7), DamageSlot{}};
  EXPECT_TRUE(damage(slots, slots, 1920, 1080).empty());
}

TEST(CsdCompositeDamage, MoveDamagesOldUnionNew) {
  const std::vector<DamageSlot> prev{armed(0, 0, 100, 100, 3)};
  const std::vector<DamageSlot> cur{armed(300, 0, 100, 100, 3)};  // same gen, moved x
  const auto d = damage(prev, cur, 1920, 1080);
...
TEST(CsdCompositeDamage, ContentGenerationChangeDamagesTheSlot) {
  const std::vector<DamageSlot> prev{armed(50, 60, 100, 100, 1)};
  const std::vector<DamageSlot> cur{armed(50, 60, 100, 100, 2)};  // repainted in place
  const auto d = damage(prev, cur, 1920, 1080);
  EXPECT_EQ(d.x, 50);
  EXPECT_EQ(d.y, 60);
  EXPECT_EQ(d.w, 100U);
...
TEST(CsdCompositeDamage, OpenDamagesNewRect_CloseDamagesOldRect) {
  // vacant -> armed
  const auto opened = damage({DamageSlot{}}, {armed(10, 20, 30, 40)}, 1920, 1080);
  EXPECT_EQ(opened.x, 10);
  EXPECT_EQ(opened.w, 30U);
  // armed -> vacant
  const auto closed = damage({armed(10, 20, 30, 40)}, {DamageSlot{}}, 1920, 1080);
...
TEST(CsdCompositeDamage, ClampsToCanvas) {
  ...
  const std::vector<DamageSlot> prev{armed(0, 0, 100, 100)};
  const std::vector<DamageSlot> cur{armed(90, 90, 100, 100)};  // extends to 190,190
